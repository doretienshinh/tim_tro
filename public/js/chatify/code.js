  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        // avatar photo
        avatar_link = data.user_avatar.split('/').pop();
        $(".messenger-infoView")
          .find(".avatar")
          .css("background-image", 'url("' + window.location.origin + '/storage/avatar/' + avatar_link + '")');
        $(".header-avatar").css(
          "background-image",
          'url("' + window.location.origin + '/storage/avatar/' + avatar_link + '")'
        );
